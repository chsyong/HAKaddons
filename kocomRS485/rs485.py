...
DEFAULT_SPEED = 'high'
# 조명 / 플러그 갯수
KOCOM_LIGHT_SIZE            = {'livingroom': 2, 'bedroom': 2, 'room1': 2, 'room2': 2, 'kitchen': 2}
...
class Kocom(rs485):
    ...
    def packet_parsing(self, packet, name='kocom', from_to='From'):
        ...
        try:
            ...
            if (v['type'] == 'ack' and v['dst_device'] == DEVICE_WALLPAD):
                ...
                if now - self.tick > KOCOM_INTERVAL / 1000:
                    try:
                        for device, d_list in self.wp_list.items():
                            if type(d_list) == dict and (device == DEVICE_PLUG and self.wp_plug):
                                for room, r_list in d_list.items():
                                    if type(r_list) == dict:
                                        if 'scan' in r_list and type(r_list['scan']) == dict and now - r_list['scan']['tick'] > SCAN_INTERVAL and (device == DEVICE_PLUG and self.wp_plug):
